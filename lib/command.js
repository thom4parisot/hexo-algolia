...
// include/exclude properties from the index
function refineProperties(config) {
  var props = DEFAULT_PROPERTIES
  return config.include ? props.concat(config.include) : props
}

module.exports = function(args, callback) {
  var hexo = this;
  var config = Object.assign({}, hexo.config.algolia);
  var indexedProperties = refineProperties(config)
  ...
  Promise.resolve(apiKey)
    .then(function(){
      if (!apiKey) {
        ...
        hexo.log.error('>> Read %s for more information.', 'https://npmjs.com/hexo-algolia#api-key');
        process.exit(1);
      }

      if (!indexName) {
        hexo.log.error('[Algolia] Please provide an Algolia index name in your hexo _config.yml file.');
        hexo.log.error('>> Read %s for more information.', 'https://npmjs.com/hexo-algolia#public-facing-search-options');
        process.exit(1);
      }
    ...
    .then(function(){
      ...
      return client.getApiKey(apiKey)
        .catch(function(err){
          ...
          hexo.log.error('>> Read %s for more information.', 'https://npmjs.com/hexo-algolia#security-concerns');
          process.exit(1);
        });
    })
    .then(function() {
    ...
    .then(function(publishedPagesAndPosts) {
      return publishedPagesAndPosts.map(function(data) {
        var storedPost = pick(data, indexedProperties);
